...
class ShipperController extends Controller
{
    ...
    public function index() {
        $shippers = AdminRole::whereArName('shipper')->first()
            ->admins()
            ->orderBy('first_name')
    ...
    public function store(Request $request) {
        if($request->post('shipper_own') == true || $request->post('shipper_own') == 1) {
            $this->validator($request);
            $role = AdminRole::whereArName('shipper')->first();
            if(!$role) {
                return sendServerErrorJsonResponse();
            }

            $admin = $this->create($request->all(), $role->ar_id);
            Mail::to($admin)->send(new AdminCreated($admin, $request->post('shipper_password')));

            if($this->createShipper($request, $admin->id)) {
                return response()->json('New Shipper created successfully..!!!', 200);
            }
        }

        if($this->createShipper($request)) {
            return response()->json('New Shipper created successfully..!!!', 200);
        }
    ...
    protected function createShipper(Request $request, int $admin_id = null) {
        ...
        $shipper->shipper_company = ($request->post('shipper_own')) ? getSiteBasic('site_title') : $request->post('shipper_company');
