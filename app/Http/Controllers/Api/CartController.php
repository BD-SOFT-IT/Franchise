...
class CartController extends Controller
{
    public function index() {
        ...
        foreach ($cart as $index => $item) {
            ...
            $cart[$index]->slug = route('product.single', ['slug' => $product->product_slug]);
    ...
    public function add(Request $request) {
        if($request->has('uid') && $request->post('uid')) {
            $cartCookie = Cookie::get('_bsoft_crt');
            $cart = json_decode($cartCookie);
            $added = false;

            foreach ($cart as $index => $item) {
                if($item->uid == $request->post('uid')) {
                    if($request->post('fran') && $request->post('fran') !== $item->fran) {
                        return response()
                            ->json([
                                'franchise' => 'You have to create separate order for purchasing from multiple vendor!'
                            ], 403);
                    }
            ...
            $cartCookie = Cookie::make('_bsoft_crt', json_encode($cart), 60 * 24 * 30, '/');
        ...
        $franchise = null;
        if($request->post('fran')) {
            $franchise = Admin::whereFranchiseId($request->post('fran'))->first();
            if(!$franchise) {
                return response()
                    ->json([
        ...
        if(!$cartCookie) {
            ...
            $cart = json_decode($cartCookie);
            foreach ($cart as $index => $item) {
                if(($franchise && !$item->fran) || ($item->fran && (!$franchise || $franchise->franchise_id !== $item->fran))) {
                    return response()
                        ->json([
                            'franchise' => 'You have to create separate order for purchasing from multiple vendor!'
                        ], 403);
    ...
    public function delete(Request $request) {
        ...
        foreach ($cart as $index => $item) {
            if($item->uid == $request->post('uid')) {
                if($item->qty == 1 || $request->post('destroy') == 1) {
                    unset($cart[$index]);
